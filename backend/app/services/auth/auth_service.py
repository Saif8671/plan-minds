            ...
            if jti and sub and iat:
                try:
                    ...
                    if redis:
                        ...
                        if revoked_all_at_str:
                            ...
                            if iat < revoked_all_at:
                                raise UnauthorizedError(
                                    "All tokens have been revoked for this user"
                                )
                    ...
                    logging.getLogger(__name__).warning(
                        f"Redis unavailable, failing open for revocation check: {e}"
                    )
        except JWTError:
    ...
    async def reset_password(self, token: str, new_password: str) -> None:
        ...
        try:
            ...
            if redis:
                ...
                await redis.setex(
                    f"user_revoked_all:{user_id}", 7 * 24 * 60 * 60, str(now)
                )
        except Exception as e:
            import logging
            ...
            # Failing the whole request is worse UX for a non-critical-path failure.
            logging.getLogger(__name__).warning(
                f"Redis unavailable, could not revoke all tokens after password reset: {e}"
            )
