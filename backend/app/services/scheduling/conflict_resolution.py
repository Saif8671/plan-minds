...
class ConflictResolutionService:
    ...
    def analyze_schedule(schedule: GeneratedSchedule) -> List[str]:
        ...
        for i in range(len(blocks) - 1):
            ...
            current_duration = (datetime.combine(schedule.date, current.end) - datetime.combine(schedule.date, current.start)).total_seconds() / 60
        ...
        if blocks:
            last = blocks[-1]
            last_duration = (datetime.combine(schedule.date, last.end) - datetime.combine(schedule.date, last.start)).total_seconds() / 60
